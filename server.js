...
app.post('/api/messages', async (req, res) => {
    ...
    console.log('Peticion recibida en /api/messages', email, 'carpeta:', folder);
    const client = new ImapFlow({
        host: host || 'imap.gmail.com',
        port: port || 993,
        secure: secure !== undefined ? secure : true,
    ...
    try {
        ...
        for await (const msg of client.fetch('1:*', { envelope: true, bodyStructure: true })) {
            ...
            try {
                const { meta, data } = await client.download(msg.uid, '1', { uid: true });
                body = data.toString().substring(0, 50000);
            } catch (e) {
                try {
                    const { meta, data } = await client.download(msg.uid, 'TEXT', { uid: true });
                    body = data.toString().substring(0, 50000);
                } catch (e2) {
                    body = '';
                }
            ...
            try {
                const { meta, data } = await client.download(msg.uid, '2', { uid: true });
                htmlBody = data.toString().substring(0, 50000);
            } catch (e) {
                htmlBody = '';
            }
            ...
            if (!body && !htmlBody) {
                try {
                    const { meta, data } = await client.download(msg.uid, 'HEADER', { uid: true });
                    body = data.toString().substring(0, 50000);
                } catch (e) {}
            }
