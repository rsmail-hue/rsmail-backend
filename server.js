...
app.post('/api/login', (req, res) => {
    ...
    const imap = new Imap({
        user: email, password, host: auto.imapHost, port: auto.imapPort, tls: true,
        tlsOptions: { rejectUnauthorized: false }
    });
    let responded = false;
    imap.once('ready', () => {
        if (!responded) {
            responded = true; imap.end();
            res.json({ success: true, message: 'Autenticación exitosa',
                account: {
                    email, password,
                    imapHost: auto.imapHost, imapPort: auto.imapPort, imapSecurity: 'ssl',
                    smtpHost: auto.smtpHost, smtpPort: auto.smtpPort, smtpSecurity: auto.smtpPort === 465 ? 'ssl' : 'starttls'
                }
            });
        }
    });
    imap.once('error', (err) => {
        if (!responded) { responded = true; imap.end(); res.status(401).json({ success: false, error: 'Credenciales inválidas: ' + err.message }); }
    });
...
app.post('/api/folders', async (req, res) => {
    ...
    const client = new ImapFlow({
        host: host || auto.imapHost, port: Number(port) || auto.imapPort, secure: true,
        auth: { user: email, pass: password }, logger: false, tls: { rejectUnauthorized: false }
    });
    try {
        ...
        res.status(500).json({ success: false, error: 'Error leyendo carpetas: ' + err.message });
    }
});

// ------------------------------------------------------------
//  OBTENER MENSAJES (ImapFlow, siempre más recientes primero)
// ------------------------------------------------------------
app.post('/api/messages', async (req, res) => {
    ...
    try {
        ...
        const messages = [];
        try {
            for await (const msg of client.fetch('1:*', { envelope: true, bodyStructure: true }, { max: limit, reverse: true })) {
                messages.push({
                    uid: msg.uid,
                    id: msg.uid.toString(),
        ...
        res.status(500).json({ success: false, error: 'Error consultando mensajes: ' + err.message });
    }
});

// ------------------------------------------------------------
//  DETALLE DEL CORREO (cuerpo HTML/texto completo)
// ------------------------------------------------------------
app.post('/api/message-detail', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, error: 'Error al abrir correo: ' + err.message });
    }
});

// ------------------------------------------------------------
//  ENVIAR CORREO (directo desde el dispositivo, backend guarda en Enviados)
// ------------------------------------------------------------
app.post('/api/save-to-sent', async (req, res) => {
    ...
    const imap = new Imap({
        user: email, password, host: imapHost, port: 993, tls: true,
        tlsOptions: { rejectUnauthorized: false }
    });
    ...
    imap.once('ready', () => {
        imap.getBoxes((err, boxes) => {
            ...
            imap.openBox(sentFolder, false, (openErr) => {
                if (openErr) { imap.end(); return res.status(500).json({ error: 'Error abriendo Enviados' }); }
...
function findSentFolder(boxes) {
    ...
    function search(boxObj, prefix = '') {
        for (const key in boxObj) {
            ...
            if (boxObj[key].attribs && boxObj[key].attribs.map(a => a.toLowerCase()).includes('\\sent')) return fullPath;
            for (const reg of patterns) if (reg.test(key) || reg.test(fullPath)) return fullPath;
            if (boxObj[key].children) { const found = search(boxObj[key].children, fullPath); if (found) return found; }
        }
        return null;
    }
    return search(boxes);
...
app.post('/api/create-folder', async (req, res) => {
    ...
    try { await client.connect(); await client.mailboxCreate(folderName); await client.logout(); res.json({ success: true }); }
    catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
...
app.post('/api/delete-folder', async (req, res) => {
    ...
    try { await client.connect(); await client.mailboxDelete(folderName); await client.logout(); res.json({ success: true }); }
    catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
